...
fun <T> directedGraph(edges: List<Pair<T, T>>): Graph<T> {
    return  edges.groupBy { it.first }.mapValues { it.value.map{it.second}.toMutableList() }.toMutableMap()
    /*
    val graph = mutableMapOf<T, MutableList<T>>()
    for ((a, b) in edges) {
        if (a !in graph) graph[a] = mutableListOf()
...
fun  Graph<Position>.shortestPath(a: Position, b: Position): Pair<Int, List<Position>> {
    ...
    while (queue.size > 0) {
        ...
        if (b == node){
            return p(distance, path)
        }

        val voisins = (this[node] ?: listOf()).sortedBy { it.second }

        for (neighbor in voisins) {
